        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Error searching entities: {response.status} - {error_text}")
                    return {
        ...
        try:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Error getting entity: {response.status} - {error_text}")
                    return {
                        "error": f"API error: {response.status}",
                        "message": error_text
                    }
                ...
                if response.status == 200:
                    ...
                    if birth_date and "results" in result:
                        ...
                        for entity in result["results"]:
                            entity_birth_date = entity.get("properties", {}).get("birthDate", [])
                            if any(bd.get("value") == birth_date for bd in entity_birth_date):
                    ...
                    error_text = await response.text()
                    logger.error(f"Error searching PEPs: {response.status} - {error_text}")
                    return {
                        "error": f"API error: {response.status}",
                        "message": error_text
                    }
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Error searching sanctions: {response.status} - {error_text}")
                    return {
                        "error": f"API error: {response.status}",
                        "message": error_text
                    }
        ...
        try:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Error getting datasets: {response.status} - {error_text}")
                    return {
                        "error": f"API error: {response.status}",
                        "message": error_text
                    }
