...
from app.legal.schemas import (
    ...
    Contract, ContractCreate, ContractUpdate, ContractVersion,
    ...
    if start_date_after or expiration_date_before:
        ...
        for contract in contracts:
            if start_date_after and contract.start_date < start_date_after:
                continue
            if expiration_date_before and contract.expiration_date and contract.expiration_date > expiration_date_before:
                continue
...
async def update_contract_endpoint(
    ...
    contract_update: ContractUpdate = Body(...)
):
    """Update a contract."""
    contract = update_contract(contract_id, contract_update.model_dump(exclude_unset=True))
    if not contract:
        raise HTTPException(status_code=404, detail="Contract not found")
    return contract
...
@router.get("/contracts/{contract_id}/versions/{version}", response_model=ContractVersion)
...
@router.put("/workflows/instances/{instance_id}/steps/{step_id}", response_model=WorkflowInstance)
